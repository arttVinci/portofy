...
import { useUploadAchievementImage } from "../hooks/mutations/achievement/useUploadImageAchievement";
...
describe("Upload Hooks Tests", () => {
  it("Achievement - should successfully call uploadImage service", async () => {
    const mockUpload = vi
      .spyOn(achievementService, "uploadImage")
      .mockResolvedValue("https://dummyimage.com/1");
    const { result } = renderHook(() => useUploadAchievementImage(), {
      wrapper,
    });
  ...
  it("Project - should successfully call uploadImage service", async () => {
    const mockUpload = vi
      .spyOn(projectService, "uploadImage")
      .mockResolvedValue("https://dummyimage.com/2");
  ...
  it("Education - should successfully call uploadImage service", async () => {
    const mockUpload = vi
      .spyOn(educationService, "uploadImage")
      .mockResolvedValue("https://dummyimage.com/3");
  ...
  it("Experience - should successfully call uploadImage service", async () => {
    const mockUpload = vi
      .spyOn(experienceService, "uploadImage")
      .mockResolvedValue("https://dummyimage.com/4");
    const { result } = renderHook(() => useUploadExperienceImage(), {
      wrapper,
    });
